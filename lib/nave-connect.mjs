// vendored from JAFairweather/luke @ 29c6926 (canonical: luke repo root) — do not edit casually.
// NGAGE EXTENSION — this copy diverges from upstream in ONE place, flagged inline:
//   localSigner() additionally exposes nip44Encrypt/nip44Decrypt (upstream's does
//   not). Ngage's gift-unwrap path (drafts.mjs receiveGrantsWithSigner) needs
//   nip44 on EVERY signer kind, so the local dev/fallback signer gains it here.
//   When re-syncing from luke, re-apply the flagged block or drafts break.
...
import { getPublicKey as pkFromSk, finalizeEvent, generateSecretKey, nip44 } from 'nostr-tools'
...
export function localSigner(sk) {
  ...
  // ── NGAGE EXTENSION (see header): nip44 primitives on the local signer, so
  // the gift-unwrap pipeline treats all three signer kinds identically.
  ...
  return {
    ...
    nip44Encrypt: async (pk, pt) => nip44.v2.encrypt(pt, conv(pk)),
    nip44Decrypt: async (pk, ct) => nip44.v2.decrypt(ct, conv(pk)),
  }
  // ── end NGAGE EXTENSION
}
...
// reconnects without re-scanning. A bare hex string is the legacy "local"
...
// it from its own key material, not from here.
